...
describe("Worker browser runtime", function suite() {
  test("parser scripts observe loading state and microtask checkpoints", async function parsing() {
    ...
    try {
      value(await browser.callTool({ name: "browser_open", arguments: { url: "https://example.test/" } }));
      assert.deepEqual(value(await browser.callTool({ name: "browser_eval", arguments: { script: "states" } })), ["loading", "microtask", "second", "interactive"]);
  ...
  test("script downloads are discovered before earlier scripts finish, preserving execution order", async function preload() {
    ...
    try {
      value(await browser.callTool({ name: "browser_open", arguments: { url: "https://example.test/" } }));
      assert.deepEqual(value(await browser.callTool({ name: "browser_eval", arguments: { script: "order" } })), ["first", ...Array(8).fill("inline"), "last"]);
    } finally { await browser.close(); }
  });
  ...
  test("retained styles refresh inheritance, selectors, tree structure and stylesheet changes", async function layout() {
    ...
    async function measure(expression = "") {
      return value(await browser.callTool({ name: "browser_eval", arguments: { script: "(()=>{" + expression + ";const r=document.querySelector('#b').getBoundingClientRect();return [r.width,r.height]})()" } }));
    }
    try {
      value(await browser.callTool({ name: "browser_open", arguments: { url: "https://example.test/" } }));
      ...
      assert.deepEqual(value(await browser.callTool({ name: "browser_eval", arguments: { script: "(()=>{const b=document.querySelector('#b');const y=b.getBoundingClientRect().y;const tail=document.createElement('div');tail.style.height='2000px';document.body.appendChild(tail);scrollTo(0,100);return[y-b.getBoundingClientRect().y,scrollY]})()" } })), [100, 100]);
    } finally { await browser.close(); }
  });
  ...
  test("computed CSS properties only measure geometry when a dimension is requested", async function computed() {
    ...
    async function evaluate(expression: string) { return value(await browser.callTool({ name: "browser_eval", arguments: { script: expression } })); }
    try {
      value(await browser.callTool({ name: "browser_open", arguments: { url: "https://example.test/" } }));
  ...
  test("background intervals do not delay hydration or a load-more click", async function scheduler() {
    ...
    try {
      value(await browser.callTool({ name: "browser_open", arguments: { url: "https://example.test/" } }));
      value(await browser.callTool({ name: "browser_click", arguments: { selector: "#more" } }));
      assert.equal(value(await browser.callTool({ name: "browser_get_text", arguments: { selector: "#out" } })), "loaded");
  ...
  test("microtasks run between timer callbacks", async function ordering() {
    ...
    try {
      value(await browser.callTool({ name: "browser_open", arguments: { url: "https://example.test/" } }));
      assert.deepEqual(value(await browser.callTool({ name: "browser_eval", arguments: { script: "order" } })), [1, 2, 3]);
    } finally { await browser.close(); }
  });
  ...
  test("pending stream closure does not poll and settles on close or error", async function streams() {
    ...
    try {
      ...
      value(await browser.callTool({ name: "browser_eval", arguments: { script: "controller.close()" } }));
      assert.equal(value(await browser.callTool({ name: "browser_eval", arguments: { script: "closed" } })), "closed");
      value(await browser.callTool({ name: "browser_open", arguments: { url: "https://example.test/" } }));
      value(await browser.callTool({ name: "browser_eval", arguments: { script: "controller.error(new Error('stream failed'))" } }));
      assert.equal(value(await browser.callTool({ name: "browser_eval", arguments: { script: "closed" } })), "stream failed");
    } finally { await browser.close(); }
  });
  ...
  test("platform subrequest failures cannot masquerade as a successful navigation", async function limits() {
    ...
    try {
      const navigation = await browser.callTool({ name: "browser_open", arguments: { url: "https://example.test/" } });
  ...
  test("snapshots and evaluation retain content beyond the former output quotas", async function output() {
    ...
    try {
      value(await browser.callTool({ name: "browser_open", arguments: { url: "https://example.test/" } }));
      const page = value(await browser.callTool({ name: "browser_snapshot", arguments: { interactive: false } })) as { snapshot: string; refs: object };
      assert.ok(page.snapshot.includes(text));
      assert.equal(Object.keys(page.refs).length, 220);
      assert.equal(value(await browser.callTool({ name: "browser_eval", arguments: { script: JSON.stringify(text) } })), text);
      value(await browser.callTool({ name: "browser_wait_for_function", arguments: { expression: "true", waitTimeoutMs: 60_000 } }));
    } finally { await browser.close(); }
  });
  ...
  test("native UTF-8 decoding preserves byte ranges, replacement, BOM and fallback encodings", async function decoding() {
    ...
    try {
      value(await browser.callTool({ name: "browser_open", arguments: { url: "https://example.test/" } }));
      const decoded = value(await browser.callTool({ name: "browser_eval", arguments: { script:
  ...
  test("tabs retain independent DOMs and release a shared browser heap on close", async function tabs() {
    ...
    try {
      await call("browser_open", { url: "https://example.test/first" });
      await call("browser_eval", { script: "globalThis.counter=7" });
      const opened = await call("browser_tab_new", { url: "https://example.test/second" }) as { tabs: { id: string; selected: boolean }[] };
      assert.equal(opened.tabs.length, 2);
      assert.equal(opened.tabs[1].selected, true);
      assert.equal(await call("browser_eval", { script: "typeof counter" }), "undefined");
      const second = await call("browser_status") as { wasmMemoryBytes: unknown };
      await call("browser_tab_switch", { tab: opened.tabs[0].id });
      assert.equal(await call("browser_eval", { script: "counter" }), 7);
      ...
      await call("browser_tab_close", { tab: opened.tabs[0].id });
      assert.equal(await call("browser_eval", { script: "document.title" }), "/second");
      await call("browser_close");
      assert.deepEqual(await call("browser_tab_list"), { tabs: [] });
      await call("browser_open", { url: "https://example.test/fresh" });
      assert.equal(await call("browser_eval", { script: "document.title" }), "/fresh");
    } finally { await browser.close(); }
  });
  ...
  test("stealth enables upstream fingerprint settings and blocks tracker subdomains", async function stealth() {
    ...
    try {
      value(await browser.callTool({ name: "browser_open", arguments: { url: "https://example.test/" } }));
      const fingerprint = value(await browser.callTool({ name: "browser_eval", arguments: { script:
  ...
  test("real script, event, fetch and DOM mutation survive MCP reconnect", async function scroll() {
    ...
    try {
      assert.equal((await browser.listTools()).tools.length, 26);
      assert.equal(requested.length, 0);
      await rpc("browser_open", { url: "https://jobs.example/" });
      assert.equal(await rpc("browser_eval", { script: "document.querySelectorAll('.vacancy').length" }), 20);
      assert.deepEqual(requested, ["https://jobs.example/"]);
      await rpc("browser_scroll", { direction: "down", amount: 1000 });
      await rpc("browser_wait_for_function", { expression: "document.querySelectorAll('.vacancy').length===21" });
      assert.equal(await rpc("browser_eval", { script: "document.querySelectorAll('.vacancy').length" }), 21);
      assert.deepEqual(requested, ["https://jobs.example/", "https://jobs.example/next"]);
      assert.equal(await rpc("browser_eval", { script: "typeof process + ':' + typeof require" }), "undefined:undefined");
    } finally { await browser.close(); }
  });
  ...
  test("DOM bridge preserves NUL separators, namespaces, fragments and script state", async function dom() {
    ...
    try {
      value(await browser.callTool({ name: "browser_open", arguments: { url: "https://example.test/" } }));
      const result = value(await browser.callTool({ name: "browser_eval", arguments: { script:
  ...
  test("CSS variable inheritance and updates survive releasing retained cascade maps", async function variables() {
    ...
    try {
      value(await browser.callTool({ name: "browser_open", arguments: { url: "https://example.test/" } }));
      const initial = value(await browser.callTool({ name: "browser_eval", arguments: { script: "(()=>{const r=document.querySelector('#box').getBoundingClientRect();return [r.width,r.height]})()" } }));
      assert.deepEqual(initial, [240, 30]);
      const updated = value(await browser.callTool({ name: "browser_eval", arguments: { script: "(()=>{document.querySelector('main').style.setProperty('--width','320px');document.documentElement.style.setProperty('--height','45px');const r=document.querySelector('#box').getBoundingClientRect();return [r.width,r.height]})()" } }));
  ...
  test("dynamic scripts, input events and cookies work across navigation", async function interaction() {
    ...
    try {
      value(await browser.callTool({ name: "browser_open", arguments: { url: "https://example.test/" } }));
      assert.equal(value(await browser.callTool({ name: "browser_eval", arguments: { script: "document.querySelector('#out').textContent" } })), "loaded");
      value(await browser.callTool({ name: "browser_fill", arguments: { selector: "#name", text: "Alice" } }));
      assert.equal(value(await browser.callTool({ name: "browser_eval", arguments: { script: "document.querySelector('#out').textContent" } })), "Alice");
      assert.equal(value(await browser.callTool({ name: "browser_eval", arguments: { script: "document.cookie" } })), "");
      value(await browser.callTool({ name: "browser_click", arguments: { selector: "a" } }));
      assert.equal(value(await browser.callTool({ name: "browser_eval", arguments: { script: "document.title" } })), "Next");
  ...
  test("actions serialize, failures recover, and close permits a fresh page", async function lifetime() {
    ...
    try {
      ...
      const results = await Promise.all([1, 2].map(function increment() {
        return browser.callTool({ name: "browser_eval", arguments: { script: "++count" } });
      }));
      ...
      assert.deepEqual(value(await browser.callTool({ name: "browser_status" })), { open: false });
      value(await browser.callTool({ name: "browser_open", arguments: { url: "https://example.test/" } }));
      assert.equal(value(await browser.callTool({ name: "browser_eval", arguments: { script: "count" } })), 0);
    } finally { await browser.close(); }
  });
  ...
  test("unbounded page JavaScript is interrupted and its page released", async function interrupt() {
    ...
    try {
      value(await browser.callTool({ name: "browser_open", arguments: { url: "https://example.test/" } }));
      const failed = await browser.callTool({ name: "browser_eval", arguments: { script: "(()=>{for(;;){} })()" } });
