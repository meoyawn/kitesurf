...
<p class="intro">Cloudflare browser automation with access reserved for you.</p>
${!signedIn ? `<section><button id="passkey">Sign in with a passkey</button><details><summary>Use your owner key</summary>
...
<p class="hint">Use the private key saved with your Kitesurf project. This is separate from your Cloudflare API token.</p><button type="submit">Sign in</button></form></details></section>`
...
<button id="register">Add a passkey</button><div class="actions"><button id="revoke" class="secondary">Revoke ChatGPT access</button><button id="logout" class="secondary">Sign out</button></div></section>`}
