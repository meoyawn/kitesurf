...
export async function mcpFetch(request: Request, browser: BrowserTools): Promise<Response> {
  const server = new Server({ name: "Kitesurf", version: "0.1.0" }, {
    capabilities: { tools: {} },
    instructions: "Browser tabs persist across tool calls and MCP reconnects. The owner shares one browser; use tabs for separate pages. Call browser_close as soon as browsing is finished to release the browser and save quota. Sessions expire after 60 seconds without browser activity. Use browser_status to inspect account limits without starting a browser. Browser launch 429s include a browserLimit diagnostic in text and structuredContent with named limits, evidence, account values, and retry guidance. Report those details and the retryAt time to the user. Treat suspectedLimit as an inference; a generic 429 alone does not prove daily quota exhaustion. A recovered retry keeps the diagnostic on the successful tool result.",
  });
